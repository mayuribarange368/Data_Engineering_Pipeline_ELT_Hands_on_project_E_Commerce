...
def transform_data(datasets):
    ...
    if 'customers' in datasets:
        ...
        processed['customers_cleaned']=customers
    ...
    if 'products' in datasets:
        ...
        products['price_category']= pd.cut(products['price'],bins=[0,50,100,500,float('inf')],
                                       labels=['Budget','Mid-range','Premium','Luxury'])

        processed['products_cleaned']=products
    ...
    if 'orders' in datasets:
        ...
        #convert total amount to numeric values
    ...
    if 'order_items' in datasets:
        ...
        processed['order_items_cleaned']=order_items
    ...
    if 'reviews' in datasets:
        ...
        processed['reviews_cleaned']=reviews
...
def upload_processed_data(s3, bucket_name, processed,metrics):
    ...
    for metric_name,df in metrics.items():
        try:
            ...
            #Upload to s3
            s3_key =f"Processed/metrics/{dataset_name}.csv" 
            s3.upload_file(local_path,bucket_name,s3_key)
...
if __name__ == "__main__":
    #Call process_ecommerce_data() and handle the results
    success = process_ecommerce_data()
    if success:
        print(f"\nNext step: Orchestration with perfect")
    else:
